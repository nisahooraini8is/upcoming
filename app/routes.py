                ...
                with Session() as session:  # Start a transaction
                    ...
                    if updatevacancy:
                        ...
                        if updatevacancy.filled_vacancy == updatevacancy.vacancy:
                            ...
                            session.add(updatevacancy)
                            session.commit()
                        # Get all active job orders for the company
                        active_joborders = session.query(Joborder).filter(
                          Joborder.company_id == companyy.id,
                          Joborder.jobstatus == 'active'
                        ).all()

                            # Check if all job orders are inactive
                        all_inactive = all(joborder.jobstatus == 'inactive' for joborder in active_joborders)
                        if all_inactive:
                                companyy.company_status = 'inactive'
                                session.add(companyy)
                                session.commit()
                        else:
                            pass
...
def jobOders():
    ...
    if show_archived:
        job_orders = Joborder.query.filter(Joborder.archived == True,Joborder.jobstatus == 'inactive')
    else:
        job_orders = Joborder.query.filter(Joborder.archived == False,Joborder.jobstatus == 'active')
